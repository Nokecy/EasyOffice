    ...
    public class FileLoader
    {
        ...
        public bool LoadFile(out string src)
        {
            ...
            if (textEncoding == null)
            {
                ...
                using (Stream stream = File.Open(path_, FileMode.Open, FileAccess.Read))
                {
                    using (StreamReader rd = new StreamReader(stream, textEncoding))
                    {
                        src = rd.ReadToEnd();
                        return true;
                    }
            ...
            else
            {
                ...
                {
                    using (Stream str = File.Open(path_, FileMode.Open, FileAccess.Read))
                    {
                        using (StreamReader rd = new StreamReader(str, textEncoding))
                        {
                            src = rd.ReadToEnd();
                            return true;
                        }
