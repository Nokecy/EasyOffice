...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlToHtmlConverter
    {
        ...
        private static int CalcWidthOfRunInTwips(XElement r)
        {
            ...
            try
            {
                ff = new System.Drawing.FontFamily(fontName);
            }
            ...
            try
            {
                using (System.Drawing.Font f = new System.Drawing.Font(ff, (float) sz/2f, fs))
                {
                    var sf = GetSizeOfString(runText, f);
            ...
            catch (ArgumentException)
            {
                try
                {
                    ...
                    using (System.Drawing.Font f = new System.Drawing.Font(ff, (float) sz/2f, fs2))
                    {
                        var sf = GetSizeOfString(runText, f);
                        const decimal dpi = 96m;
                        var twip = (int)((((int)sf.Width / dpi) * 1440m) / multiplier + tabLength * 1440m);
                ...
                catch (ArgumentException)
                {
                    ...
                    try
                    {
                        using (var f = new System.Drawing.Font(ff, (float) sz/2f, fs2))
                        {
                            var sf = GetSizeOfString(runText, f);
                            const decimal dpi = 96m;
                            var twip = (int)((((int)sf.Width / dpi) * 1440m) / multiplier + tabLength * 1440m);
                    ...
                    catch (ArgumentException)
                    {
                        ...
                        var ff2 = new System.Drawing.FontFamily("Times New Roman");
                        using (var f = new System.Drawing.Font(ff2, (float) sz/2f, fs))
                        {
                            var sf = GetSizeOfString(runText, f);
                            const decimal dpi = 96m;
                            var twip = (int)((((int)sf.Width / dpi) * 1440m) / multiplier + tabLength * 1440m);
        ...
        private static SizeF GetSizeOfString(string str, System.Drawing.Font font)
        {
        ...
        private static void GenerateBorderStyle(XElement pBdr, XName sideXName, Dictionary<string, string> style, BorderType borderType)
        {
            ...
            {
                var sz = side.Attribute(W.sz) == null ? 1 : (int)side.Attribute(W.sz);
                var space = (decimal?)side.Attribute(W.space) ?? 0;
